import { copyText } from "@repo/shared";
...
function updateMainButtonText(mainButton: HTMLDivElement, text: string) {
    ...
    for (const option of prefixes) {
      ...
      item.addEventListener("click", () => {
        ...
        const link = option.isDirectLink
          ? currentUrl
          : option.prefix.includes("{url}") || option.prefix.includes("{BV}")
            ? option.prefix
